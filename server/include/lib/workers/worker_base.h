...
class WorkerBase {
protected:
    std::unique_ptr<std::queue<RequestType>> request_queue;
    ...
    void initialize_queue() {
        request_queue = std::make_unique<std::queue<RequestType>>();
    }

    RequestType pop_request() {
        RequestType req = request_queue->front();
        request_queue->pop();
        return req;
    }
    ...
    void process_data() {
        ...
        std::cout << "Stopped WorkerBase..." << std::endl;
    }
...
public:
    ...
    void start_processing() {
        processing_thread = std::thread(&WorkerBase::process_data, this);
    }
